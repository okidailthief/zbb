    ...
    <>
      ...
      >
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-6">
          ...
          {/* Mobile hamburger */}
          <button
            className="md:hidden text-[#fafaf9] p-2 relative z-[60]"
      ...
      {/* Mobile overlay menu — rendered outside <header> to avoid backdrop-filter containing block */}
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-8 bg-[#1c1917]/98 backdrop-blur-lg md:hidden"
          >
